...
impl Solution {
    pub fn count_mentions(number_of_users: i32, events: Vec<Vec<String>>) -> Vec<i32> {
        ...
        let mut user_status: Vec<bool> = vec![true; number_of_users];
        let mut user_mention: Vec<i32> = vec![0; number_of_users];

        let mut all_count = 0;
        let mut last_msg_id = 0;

        let mut msg_map: BTreeMap<(usize, EventType), Vec<usize>> = BTreeMap::new();
        ...
        for event in events {
            ...
            let event_timestamp = event[1].parse::<usize>().unwrap();
            ...
            msg_map.entry((event_timestamp, event_type))
                .and_modify(|v| v.extend(&mentions.clone()))
                .or_insert(mentions.clone());

            if event_type == EventType::OFFLINE {
                msg_map.entry((event_timestamp + 60, EventType::ONLINE))
                    ...
                    for &id in msg_target.iter() {
                        if id == usize::MAX {
                            ...
                            for (user_id, &status) in  user_status.iter().enumerate() {
                                user_mention[user_id] += (status as i32)
                            }
                        } else {
                            user_mention[id] += 1;
                        }
